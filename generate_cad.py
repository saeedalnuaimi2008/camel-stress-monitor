...
# Define dimensions in millimeters for readability, then convert to meters
# Enclosure targets standard ESP32 dev board + DHT22 + MPU6050 stack
LENGTH_MM = 120.0
WIDTH_MM = 80.0
HEIGHT_MM = 50.0
WALL_THICKNESS = 2.0
...
inner_box = trimesh.creation.box(extents=[l_ext - offset, w_ext - 
offset, h_ext - offset])
    
shell = outer_box.difference(inner_box)
shell.visual.vertex_colors = [235, 235, 235, 255] # Matte white ASA 
    
    # Inner core (PETG insert)
...
core_cavity = trimesh.creation.box(extents=[core_l - mm_to_m(4), 
core_w - mm_to_m(4), core_h - mm_to_m(4)])
inner_housing = core.difference(core_cavity)
inner_housing.visual.vertex_colors = [45, 45, 45, 255] # Dark charcoal 
"PETG"
    
    # Scene assembly
scene = trimesh.Scene()
scene.add_geometry(shell, node_name="outer_shield_ASA")
